...
int FusionCameraDetectionComponent::InternalProc(
  ...
  cv::Mat cv_img(image_height_, image_width_, CV_8UC3, data, static_cast<size_t>(0));
 // drawing bounding boxes - green
  for (auto obj : camera_frame.tracked_objects) {
    auto &box = obj->camera_supplement.box;
    if (box.xmin < box.xmax && box.ymin < box.ymax) {

      cv::Point pt1(static_cast<int>(box.xmax), static_cast<int>(box.ymax));
      cv::Point pt2(static_cast<int>(box.xmin), static_cast<int>(box.ymin));
  ...
  for(auto lane : camera_frame.lane_objects){
    ...
    //AERROR<<"X start, x end "<<x_start<<" "<<x_end;
    ...
    double iter_x = (x_end - x_start) / static_cast<double>(num_of_points);
    double prev_x = 0.0;
    double prev_y = 0.0;
    ...
    for(int i = 0; i < num_of_points; i++){
      ...
      // draw line
        if(i != 0){
          // AERROR<<"prev x y "<<prev_x<<" "<<prev_y<<" , current x y "<<x_start<<" y_current"<<y_current;
          cv::Point pt1_(static_cast<int>(prev_y), static_cast<int>(prev_x));
          cv::Point pt2_(static_cast<int>(y_current), static_cast<int>(x_start));
